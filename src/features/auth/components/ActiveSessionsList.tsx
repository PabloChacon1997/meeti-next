...
export default async function ActiveSessionsList() {
  ...
  return (
    <>
      <Heading level={2} className="mt-10">Sesiones Activas</Heading>
          ...
          sessions.map(session => (
            <div key={session.id} className="p-5 shadow-xs flex items-center">
              <div className="flex gap-2 items-center flex-1">
                ...
                {isCurrentDevice(session.id) && <p className="text-green-600 font-bold bg-green-200 border border-green-200 rounded-sm inline-block px-3 py-1 uppercase textxs">Este dispositivo</p>}
              </div>
            </div>
          ))
        }
